...
from lib.decoders import CASCADE_Add, CASCADE_Cat
...
class MERIT_Cascaded(nn.Module):
    ...
    def forward(self, x):
        ...
        # decoder 将tranformer的输入加到convnext中，送到deconde中
        x11_o, x12_o, x13_o, x14_o = self.decoder(ff[3] + f1[3], [ff[2] + f1[2], ff[1]+f1[1], ff[0]+f1[0]])

        #将convnext送入到decode中
        x21_o, x22_o, x23_o, x24_o = self.decoder(ff[3], [ff[2], ff[1], ff[0]])
